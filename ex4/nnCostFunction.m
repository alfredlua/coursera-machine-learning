...
for i = 1:m
    ...
    label(y(i)) = 1;
    err = 1 / m * (-label' .* log(hx(i,:)) - (1-label)' .* log(1-hx(i,:)));
    J = J + sum(err);
end
...
for t = 1:m
    ...
    Theta1_grad = Theta1_grad + (delta2 * a1'); % 25x401 (matches Theta1 size)
    Theta2_grad = Theta2_grad + (delta3 * a2'); % 10*26 (matches Theta2 size)
end
...
% Set first column of Theta (for the bias node) to zero
Theta1_reg = [zeros(size(Theta1, 1), 1) Theta1(:, 2:end)]; % 25x401
Theta2_reg = [zeros(size(Theta2, 1), 1) Theta2(:, 2:end)]; % 10x26
